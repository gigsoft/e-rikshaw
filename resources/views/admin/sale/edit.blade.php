                    ...
                    <div class="row">
                        <table class="table table-bordered">
                            ...
                            @foreach($saleDetails as $key => $detail)
                                <tr>
                                    <td>
                                        <select name="invoice[{{ $key }}][vehicle_models]" class="form-select form-control vehicle_models" required>
                                            <option value="">Select Model </option>
                                            @foreach ($vehicle_models as $row)
                                            <option value="{{ $row->id }}" {{ $row->id == $detail->vehicle_id ? 'selected' : '' }}>
                                                {{$row->name}}
                                            @endforeach
                                        </select>
                                    </td>
                                    ...
                                    <td>
                                        <input type="text" id="total_{{ $key }}" name="invoice[{{ $key }}][total]" class="form-control required invoice_value invoice-item" required value="{{ $detail->total_price }}">
                                    </td>
                                    ...
                                    <td>
                                        <select name="invoice[{{ $key }}][status]" class="form-select form-control selected_item" required>
                                            ...
                                            <option value="2" {{ $detail->status == 2 ? 'selected' : '' }}>InComplete</option>
                                        </select>
                                    </td>
                                    <td>
...
<script>
    ...
    function calculateTotal(inputField) {
        ...
        // Add a new row
        $(document).on('change', '.vehicle_models', function() {
        var selectedModelId = $(this).val();
        var selectedModelName = $(this).find('option:selected').text();
        var $row = $(this).closest('tr');
        $row.find('.model-name').text(selectedModelName);

        // If you need to store the selected model ID in a hidden input, you can do it like this:
        // $row.find('.model-id-input').val(selectedModelId);
    });
    ...
    $.ajax({
        type: 'POST',
        url: '/sale/update/' + invoiceId, // Use your existing route for updating purchases
        data: {
