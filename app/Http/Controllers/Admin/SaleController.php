...
//this function use Data Show in Index Table
    public function index(){
        ...
        foreach($sale_headers as $value){
            ...
            $value->incomplete_count      = Sale_Details::where('order_id',$value->id)->where('status',"2")->count();
        ...
        if ($validator->fails()) {
            ...
            return redirect()->back()->withErrors($validator)->withInput();
               ...
               foreach ($numRows as $key => $value) {
                // Check if 'vehicle_models' key exists in the $value array
                $vehicleId = isset($value['vehicle_models']) ? $value['vehicle_models'] : null;
                ...
                $detailData = [
                    ...
                    'quantity'           => $value['qty'],
                    'price'              => $value['price'],
                    'vehicle_id'         => $vehicleId, // Assign the value or null if the key doesn't exist
...
// this function use in Show Item Data
    ...
    public function update(Request $request, $id)
   {
         ...
         if ($validator->fails()) {
            ...
            // Create Sale_Details records
            Sale_Details::create($detailData);
        }
    }

    return redirect()->route('admin.sale')->with('success', 'Items updated successfully.');
 }
